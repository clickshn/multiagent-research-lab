...
**두 축을 모두 재기로 한 것은 사람이 세운 설계이고, 그 설계에는 전제가 하나 붙어 있었다** —
`release_type` arm은 한국어 단문을 대량 제거하고 `tech_domain` arm은 남기므로,
**두 arm의 개선 차이가 곧 오염 제거분**이라는 것이었다. 배선의 목적 자체가 그 뺄셈이었다.
**측정 결과 그 전제는 기각됐다**(Rationale 5, Evidence). 배선 설계는 그대로 유효하고,
바뀐 것은 **두 arm의 차이를 무엇으로 읽느냐**다.
...
5. ⚠️ **"두 arm의 차이 = 오염 제거분"이라는 설계 전제는 실측으로 기각됐고, 그 자리를
   대조군 논거가 대신한다.** 이 arm 설계는 원래 **뺄셈**으로 세워져 있었다 —
   `release_type` arm은 한국어 단문을 대량 제거하고 `tech_domain` arm은 남기므로,
   두 arm의 개선 차이가 곧 오염 제거분이라는 것이었다. **그 비대칭이 존재하지 않는다**
   (Evidence). 두 축이 제거하는 한국어 단문은 케이스당 **8.57 vs 8.43건으로 사실상 같고**,
   차이는 영어 장문 제거량(1.71 vs 6.07)에서 난다. **뺄셈은 성립하지 않는다.**

   **대신 성립한 논거는 더 직접적이다:** `tech_domain` strict는 한국어 단문을 **케이스당
   평균 1.57건 남긴 채**(`release_type` arm의 1.43건보다 **많다**) 층 A 재현율
   **14/14 = 100%**를 냈다. 주력 케이스 GS-010은 **오염원(`news:3dd7208e`, Dream-RSI)이
   1위에 그대로 있는 상태**에서 기대 문서가 19위 → 2위로 올라와 top-4에 들어왔다.

   > **오염원이 남은 채로 이긴 것이, 두 arm을 빼서 얻은 잔차보다 강한 증거다.**
   > 뺄셈은 "제거량이 서로 다르다"는 전제와 "차이가 오직 오염에서 온다"는 전제를
   > **둘 다** 요구한다. 잔존 대조는 전제를 요구하지 않는다 — **오염원이 화면에 남아
   > 있는데도 기대 문서가 그 위로 올라왔다면**, 그 개선은 오염 제거일 수 없다.
   > 전제를 하나 잃은 것이 아니라, 전제가 필요 없는 논거로 바뀐 것이다.

   ⚠️ **다만 잔존 대조가 성립하는 것은 층 A 14건 중 10건이다.** `tech_domain` strict에서
   한국어 단문 잔존이 0건인 GS-013·016·018·021은 **개선이 오염 제거와 구분되지 않는다.**
   골든셋 GS-013의 `warning`이 미리 적어 둔 그대로이며, ③ 대조군 판정은 나머지 10건으로 한다.
...
- **Experiment (설계 전제의 기각):** 층 A 14건 케이스당 평균 제거 건수 —
  한국어 단문은 `release_type` strict **8.57 / 10**, `tech_domain` strict **8.43 / 10**으로
  거의 같다. 두 축을 가르는 것은 **신규 영어 장문**(1.71 vs **6.07**)이다.
  한국어 단문 **잔존**은 `release_type` 1.43건 / `tech_domain` **1.57건**(잔존 ≥1인 케이스
  14건 중 **10건**). → Rationale 5의 근거다.

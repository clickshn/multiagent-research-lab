...
- **엔드포인트에 인증이 없다.** URL을 아는 누구나 호출할 수 있다. URL 커밋 금지는
  접근 통제가 아니라 노출 억제일 뿐이다. 사내 정책상 이대로 괜찮은지 확인 필요.
  (`VLLM_API_KEY` 키는 `.env.example`에 미리 열어뒀다.)
...
- **CLAUDE.md가 여전히 `.gitignore`에 있다.** session-01에서 제기했으나 미해결.
  이번 세션에서 **승인 규칙을 바꿨는데 그 변경이 버전 관리되지 않는다** — 팀원은 낡은
  규칙을 보게 되고, 승인 게이트처럼 중요한 규칙이 커밋 이력에 남지 않는다.
  session-01보다 리스크가 커졌으니 의도된 것인지 확인 바란다.

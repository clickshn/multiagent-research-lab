...
**방향에 따라 위험이 다르다. 이게 규칙이다.**

| 변경 방향 | 전파 전에 관측되는 것 | 위험 |
| --- | --- | --- |
| 권한을 **뺀다** (`detach`) | 아직 되는 것처럼 보인다 | ⚠️ **거짓 통과 — 검증으로 쓸 수 없다** |
| 권한을 **더한다** (정책 버전 추가) | 아직 안 되는 것처럼 보인다 | 거짓 실패 — 해롭지 않다, 다시 돌리면 된다 |

> **`detach` 직후의 성공은 검증이 아니다.** 시간을 두고 한 번 더 돌린 결과만 믿는다.
> 반대로 **권한을 더한 뒤의 성공은 그대로 믿어도 된다.**
...
- **`detach` 순서 정렬은 이름 기반 휴리스틱이다.** IAM 권한을 주는 정책
  (`IAM|Admin|ActionsWithAWSResourceControl`)을 마지막에 떼도록 고쳤지만, 어떤 관리형
  정책이 실제로 `iam:DetachUserPolicy`를 주는지 확인하려면 정책 문서를 읽어야 한다.
  최소 권한 정책에는 `iam:GetPolicyVersion`이 없다(일부러 뺐다). **새로운 이름 패턴은
  스크립트의 `IAM_GRANTING_RE`에 직접 추가해야 한다.**
